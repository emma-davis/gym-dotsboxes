...
# - Fixes to order of players, allowing A or B to go first #
...
# ~~~ ENVIRONMENT CLASS ~~~
class DotsBoxesEnv(gym.Env):  # ADD PARAM TO DEFINE START MARK, EITHER A OR B
    metadata = {'render.modes': ['human']}

    def __init__(self, start_mark, show_number=False):
        self.num_actions = NUM_ACTIONS
        self.action_space = [i for i in range(NUM_ACTIONS)]
        self.available_actions = [i for i in range(NUM_ACTIONS)]
        self.observation_space = spaces.Discrete(NUM_ACTIONS)
        self.state_space = 2 ** NUM_ACTIONS
        self.set_start_mark(start_mark)
        self.start_state = np.random.randint(0, NUM_ACTIONS)  # NOT SURE ON THIS, CHANGE LATER
    ...
    def enable_action(self, action):
        # CALLED TO ADD ACTIONS BACK INTO WHAT IS ALLOWED, THUS ADDING TO ACTION SPACE
        self.available_actions = self.available_actions.append(action)
    ...
    def step(self, action):
        ...
        if (to_num(self.mark) == 1) & (a_total - a_old_total > 0):
            reward = 10  # REWARD IS KINDA REDUNDANT ATM
